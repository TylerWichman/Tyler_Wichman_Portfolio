...
def compute_rookie_college_features(draft_capital_df: pd.DataFrame, draft_year: int = 2026) -> pd.DataFrame:
    ...
    if not os.environ.get("CFBD_API_KEY"):
        print("[cfbd_rookie] CFBD_API_KEY is not set at all (not just exhausted) -- skipping CFBD "
              "college-production lookups entirely for this run. Rookies fall back to draft "
              "capital + age (+ combine speed score for RBs) only. Set CFBD_API_KEY (see README) "
              "to enable this feature.")
        return pd.DataFrame({
            "gsis_id": rookies["gsis_id"].values,
            "dominator_rating": np.nan,
            "breakout_age": np.nan,
